...
bool gAsyncText{false};
bool gMultiline{false};
} // namespace

// This example shows the blur radius property of the color visual and animates it.
//
class TextLabelBenchmarkExample : public ConnectionTracker
{
public:
  TextLabelBenchmarkExample(Application& application)
  : mApplication(application)
  {
    // Connect to the Application's Init signal
    mApplication.InitSignal().Connect(this, &TextLabelBenchmarkExample::Create);
  }

  ~TextLabelBenchmarkExample()
  {
    // Nothing to do here;
  }
    ...
    int i = n - 1;
    {
      ...
      for(int j = 0; j < m; j++)
      {
        ...
        customNew1Logger.AddMarker(PerformanceLogger::Marker::START_EVENT);
        {
          ...
          if(gAsyncText)
          {
            bgView[DevelTextLabel::Property::RENDER_MODE] = DevelTextLabel::Render::Mode::ASYNC_AUTO;
          }

          if(gMultiline)
          {
            bgView[TextLabel::Property::MULTI_LINE] = true;
          }
        }
        customNew1Logger.AddMarker(PerformanceLogger::Marker::END_EVENT);
...
int DALI_EXPORT_API main(int argc, char** argv)
{
  ...
  TextLabelBenchmarkExample test(application);
