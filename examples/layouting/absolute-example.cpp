...
void AbsoluteExample::Create()
{
  ...
  mAbsoluteLayout = Control::New();

  // Create HBoxLayout for this control.
  auto hboxLayout = AbsoluteLayout::New();
  DevelControl::SetLayout( mAbsoluteLayout, hboxLayout );
  mAbsoluteLayout.SetName("AbsoluteLayout");

  mAbsoluteLayout.SetAnchorPoint( AnchorPoint::CENTER );
  mAbsoluteLayout.SetParentOrigin( ParentOrigin::CENTER );

  stage.Add( mAbsoluteLayout );
  ...
  for( unsigned int x = 0; x < NUMBER_OF_IMAGE_VIEWS; x++ )
  {
    ...
    mAbsoluteLayout.Add( mImageViews[x] );

    mImageViewToggleStatus[ x ] = true;
  }

  mImageViews[1].SetProperty( Dali::Actor::Property::POSITION, Vector3( 100.0f, 0.0f, 0.0f) );
  mImageViews[2].SetProperty( Dali::Actor::Property::POSITION, Vector3( 0.0f, 100.0f, 0.0f) );
  mImageViews[3].SetProperty( Dali::Actor::Property::POSITION, Vector3( 200.0f, 200.0f, 0.0f) );
}

void AbsoluteExample::Remove()
{
  UnparentAndReset( mAbsoluteLayout );
}
