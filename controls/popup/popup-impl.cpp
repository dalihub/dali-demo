...
BaseHandle CreateToast()
{
  ...
  popup.SetProperty(Demo::Popup::Property::CONTEXTUAL_MODE, Demo::Popup::NON_CONTEXTUAL);
  popup.SetProperty(Demo::Popup::Property::ANIMATION_DURATION, DEFAULT_TOAST_TRANSITION_TIME);
  popup.SetProperty(Demo::Popup::Property::TAIL_VISIBILITY, false);

  // Disable the dimmed backing.
  popup.SetProperty(Demo::Popup::Property::BACKING_ENABLED, false);

  // The toast popup should fade in (not zoom).
  popup.SetProperty(Demo::Popup::Property::ANIMATION_MODE, Demo::Popup::FADE);

  // The toast popup should auto-hide.
  popup.SetProperty(Demo::Popup::Property::AUTO_HIDE_DELAY, DEFAULT_TOAST_AUTO_HIDE_DELAY);
  ...
  popup.SetProperty(Demo::Popup::Property::TOUCH_TRANSPARENT, true);
...
DALI_TYPE_REGISTRATION_BEGIN(Demo::Popup, Toolkit::Control, Create )

// Main content related properties.
DALI_PROPERTY_REGISTRATION(Demo, Popup, "title",                 MAP,       TITLE                  )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "content",               MAP,       CONTENT                )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "footer",                MAP,       FOOTER                 )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "displayState",          STRING,    DISPLAY_STATE          )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "touchTransparent",      BOOLEAN,   TOUCH_TRANSPARENT      )

// Contextual related properties.
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailVisibility",        BOOLEAN,   TAIL_VISIBILITY        )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailPosition",          VECTOR3,   TAIL_POSITION          )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "contextualMode",        STRING,    CONTEXTUAL_MODE        )

// Animation related properties.
DALI_PROPERTY_REGISTRATION(Demo, Popup, "animationDuration",     FLOAT,     ANIMATION_DURATION     )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "animationMode",         STRING,    ANIMATION_MODE         )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "entryAnimation",        MAP,       ENTRY_ANIMATION        )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "exitAnimation",         MAP,       EXIT_ANIMATION         )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "autoHideDelay",         INTEGER,   AUTO_HIDE_DELAY        )

// Style related properties.
DALI_PROPERTY_REGISTRATION(Demo, Popup, "backingEnabled",        BOOLEAN,   BACKING_ENABLED        )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "backingColor",          VECTOR4,   BACKING_COLOR          )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "popupBackgroundImage",  STRING,    POPUP_BACKGROUND_IMAGE )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "popupBackgroundBorder", EXTENTS, POPUP_BACKGROUND_BORDER)
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailUpImage",           STRING,    TAIL_UP_IMAGE          )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailDownImage",         STRING,    TAIL_DOWN_IMAGE        )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailLeftImage",         STRING,    TAIL_LEFT_IMAGE        )
DALI_PROPERTY_REGISTRATION(Demo, Popup, "tailRightImage",        STRING,    TAIL_RIGHT_IMAGE       )

// Signals.
DALI_SIGNAL_REGISTRATION(Demo, Popup, "touchedOutside", SIGNAL_TOUCHED_OUTSIDE)
DALI_SIGNAL_REGISTRATION(Demo, Popup, "showing",        SIGNAL_SHOWING        )
DALI_SIGNAL_REGISTRATION(Demo, Popup, "shown",          SIGNAL_SHOWN          )
DALI_SIGNAL_REGISTRATION(Demo, Popup, "hiding",         SIGNAL_HIDING         )
DALI_SIGNAL_REGISTRATION(Demo, Popup, "hidden",         SIGNAL_HIDDEN         )
...
TypeRegistration typeRegistrationToast("PopupToast",  typeid( Demo::Popup ), CreateToast);

// Enumeration to / from string conversion tables

const Scripting::StringEnum DisplayStateTable[] =
{
  {"SHOWING", Demo::Popup::SHOWING},
  {"SHOWN",   Demo::Popup::SHOWN  },
  {"HIDING",  Demo::Popup::HIDING },
  {"HIDDEN",  Demo::Popup::HIDDEN },
};
...
{
  {"NONE",    Demo::Popup::NONE  },
  {"ZOOM",    Demo::Popup::ZOOM  },
  {"FADE",    Demo::Popup::FADE  },
  {"CUSTOM",  Demo::Popup::CUSTOM},
};
...
{
  {"NON_CONTEXTUAL", Demo::Popup::NON_CONTEXTUAL},
  {"ABOVE",          Demo::Popup::ABOVE         },
  {"RIGHT",          Demo::Popup::RIGHT         },
  {"BELOW",          Demo::Popup::BELOW         },
  {"LEFT",           Demo::Popup::LEFT          },
};
...
Demo::Popup Popup::New()
{
  // Create the implementation
  PopupPtr popup(new Popup());

  // Pass ownership to CustomActor via derived handle.
  Demo::Popup handle(*popup);
...
: ControlImpl(ControlBehaviour(CONTROL_BEHAVIOUR_DEFAULT)),
  ...
  mContent(),
  mFooter(),
  mDisplayState(Demo::Popup::HIDDEN), // Hidden until shown with SetDisplayState()
  mTailVisible(false),
  mTailPosition(DEFAULT_TAIL_POSITION),
  mContextualMode(Demo::Popup::NON_CONTEXTUAL),
  mAnimationDuration(DEFAULT_POPUP_ANIMATION_DURATION),
  mAnimationMode(Demo::Popup::FADE),
  mEntryAnimationData(),
  mExitAnimationData(),
  mAutoHideDelay(0),
  ...
  mTailRightImage()
{
  ...
  const std::string imageDirPath = DEMO_STYLE_DIR "images/";
...
void Popup::OnInitialize()
{
  ...
  self.SetProperty(Toolkit::DevelControl::Property::ACCESSIBILITY_ROLE, Toolkit::DevelControl::AccessibilityRole::DIALOG);
}

Toolkit::DevelControl::ControlAccessible* Popup::CreateAccessibleObject()
{
...
void Popup::DisplayStateChangeComplete()
{
  ...
  if(mDisplayState == Demo::Popup::HIDING)
  {
    mDisplayState = Demo::Popup::HIDDEN;

    mLayer.SetProperty(Actor::Property::VISIBLE, false);
    mPopupLayout.SetProperty(Actor::Property::SENSITIVE, false);

    // Guard against destruction during signal emission.
    Demo::Popup handle(GetOwner());
    mHiddenSignal.Emit();
  }
  else if(mDisplayState == Demo::Popup::SHOWING)
  {
    mDisplayState = Demo::Popup::SHOWN;
    Demo::Popup handle(GetOwner());
    mShownSignal.Emit();
...
bool Popup::OnAutoHideTimeReached()
{
  if(!Dali::Accessibility::IsUp() || true) // TODO: remove 'true' in sync with EFL (UX change)
  {
    ...
    SetDisplayState(Demo::Popup::HIDDEN);
  }
...
void Popup::SetDisplayState(Demo::Popup::DisplayState displayState)
{
  ...
  bool display = (displayState == Demo::Popup::SHOWING) || (displayState == Demo::Popup::SHOWN);

  // Ignore if we are already at the target display state.
  if(display == ((mDisplayState == Demo::Popup::SHOWING) || (mDisplayState == Demo::Popup::SHOWN)))
  {
    return;
  }

  // Convert the bool state to the actual display state to use.
  mDisplayState = display ? Demo::Popup::SHOWING : Demo::Popup::HIDING;

  if(display)
  {
    // Update the state to indicate the current intent.
    mDisplayState = Demo::Popup::SHOWING;
    Toolkit::DevelControl::EmitAccessibilityStateChanged(Self(), Accessibility::State::SHOWING, 1);
  ...
  {
    mDisplayState = Demo::Popup::HIDING;
    ClearKeyInputFocus();
    Toolkit::DevelControl::EmitAccessibilityStateChanged(Self(), Accessibility::State::SHOWING, 0);
...
void Popup::SetContextualMode(Demo::Popup::ContextualMode mode)
{
  mContextualMode = mode;
  mLayoutDirty    = true;
}

Demo::Popup::ContextualMode Popup::GetContextualMode() const
{
...
Demo::Popup::TouchedOutsideSignalType& Popup::OutsideTouchedSignal()
{
  return mTouchedOutsideSignal;
}

Demo::Popup::DisplayStateChangeSignalType& Popup::ShowingSignal()
{
  return mShowingSignal;
}

Demo::Popup::DisplayStateChangeSignalType& Popup::ShownSignal()
{
  return mShownSignal;
}

Demo::Popup::DisplayStateChangeSignalType& Popup::HidingSignal()
{
  return mHidingSignal;
}

Demo::Popup::DisplayStateChangeSignalType& Popup::HiddenSignal()
{
...
void Popup::SetAnimationMode(Demo::Popup::AnimationMode animationMode)
{
  mAnimationMode = animationMode;
  mLayoutDirty   = true;
}

Demo::Popup::AnimationMode Popup::GetAnimationMode() const
{
...
{
  Demo::Popup popup = Demo::Popup::DownCast(Dali::BaseHandle(object));
  ...
  if(popup)
  {
    ...
    {
      case Demo::Popup::Property::TITLE:
      {
      ...
      case Demo::Popup::Property::CONTENT:
      {
        Property::Map valueMap;
        if(value.Get(valueMap))
        {
      ...
      case Demo::Popup::Property::FOOTER:
      {
        Property::Map valueMap;
        if(value.Get(valueMap))
        {
      ...
      case Demo::Popup::Property::DISPLAY_STATE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
          Demo::Popup::DisplayState displayState(Demo::Popup::HIDDEN);
          if(Scripting::GetEnumeration<Demo::Popup::DisplayState>(valueString.c_str(), DisplayStateTable, DisplayStateTableCount, displayState))
          {
      ...
      case Demo::Popup::Property::TOUCH_TRANSPARENT:
      {
        bool valueBool;
      ...
      case Demo::Popup::Property::TAIL_VISIBILITY:
      {
        bool valueBool;
        if(value.Get(valueBool))
        {
      ...
      case Demo::Popup::Property::TAIL_POSITION:
      {
      ...
      case Demo::Popup::Property::CONTEXTUAL_MODE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
          Demo::Popup::ContextualMode contextualMode(Demo::Popup::BELOW);
          if(Scripting::GetEnumeration<Demo::Popup::ContextualMode>(valueString.c_str(), ContextualModeTable, ContextualModeTableCount, contextualMode))
          {
      ...
      case Demo::Popup::Property::ANIMATION_DURATION:
      {
        float valueFloat;
      ...
      case Demo::Popup::Property::ANIMATION_MODE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
          Demo::Popup::AnimationMode animationMode(Demo::Popup::FADE);
          if(Scripting::GetEnumeration<Demo::Popup::AnimationMode>(valueString.c_str(), AnimationModeTable, AnimationModeTableCount, animationMode))
          {
      ...
      case Demo::Popup::Property::ENTRY_ANIMATION:
      {
        Property::Map valueMap;
        if(value.Get(valueMap))
        {
      ...
      case Demo::Popup::Property::EXIT_ANIMATION:
      {
        Property::Map valueMap;
        if(value.Get(valueMap))
        {
      ...
      case Demo::Popup::Property::AUTO_HIDE_DELAY:
      {
        int valueInt;
      ...
      case Demo::Popup::Property::BACKING_ENABLED:
      {
        bool valueBool;
        if(value.Get(valueBool))
        {
      ...
      case Demo::Popup::Property::BACKING_COLOR:
      {
      ...
      case Demo::Popup::Property::POPUP_BACKGROUND_IMAGE:
      {
      ...
      case Demo::Popup::Property::POPUP_BACKGROUND_BORDER:
      {
      ...
      case Demo::Popup::Property::TAIL_UP_IMAGE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
      ...
      case Demo::Popup::Property::TAIL_DOWN_IMAGE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
      ...
      case Demo::Popup::Property::TAIL_LEFT_IMAGE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
      ...
      case Demo::Popup::Property::TAIL_RIGHT_IMAGE:
      {
        std::string valueString;
        if(GetStdString(value, valueString))
        {
...
Property::Value Popup::GetProperty(BaseObject* object, Property::Index propertyIndex)
{
  ...
  if(popup)
  {
    ...
    switch(propertyIndex)
    {
      ...
      case Demo::Popup::Property::DISPLAY_STATE:
      {
        value = Scripting::GetLinearEnumerationName<Demo::Popup::DisplayState>(popupImpl.GetDisplayState(), DisplayStateTable, DisplayStateTableCount);
        break;
      }
      case Demo::Popup::Property::TOUCH_TRANSPARENT:
      {
      ...
      case Demo::Popup::Property::CONTEXTUAL_MODE:
      {
        value = Scripting::GetLinearEnumerationName<Demo::Popup::ContextualMode>(popupImpl.GetContextualMode(), ContextualModeTable, ContextualModeTableCount);
        break;
      }
      case Demo::Popup::Property::ANIMATION_DURATION:
      {
      ...
      case Demo::Popup::Property::ANIMATION_MODE:
      {
        value = Scripting::GetLinearEnumerationName<Demo::Popup::AnimationMode>(popupImpl.GetAnimationMode(), AnimationModeTable, AnimationModeTableCount);
        break;
      }
      case Demo::Popup::Property::ENTRY_ANIMATION:
      {
...
bool Popup::DoConnectSignal(BaseObject* object, ConnectionTrackerInterface* tracker, const Dali::String& signalName, FunctorDelegate* functor)
{
  ...
  bool        connected(true);
  Demo::Popup popup = Demo::Popup::DownCast(handle);
...
void Popup::LayoutContext(const Vector2& size)
{
  ...
  if((mContextualMode == Demo::Popup::NON_CONTEXTUAL) || !parent)
  {
    return;
  }
...
Actor Popup::GetNextKeyboardFocusableActor(Actor currentFocusedActor, Toolkit::Control::KeyboardFocus::Direction direction, bool loopEnabled)
{
  ...
  if(currentFocusedActor)
  {
    currentFocusGroup = Dali::Toolkit::KeyboardFocusManager::Get().GetFocusGroup(currentFocusedActor);
  }
...
{
  auto        popup = Demo::Popup::DownCast(Self());
  std::string title;
...
{
  auto states       = Toolkit::DevelControl::ControlAccessible::CalculateStates();
  auto popup        = Demo::Popup::DownCast(Self());
  auto displayState = ToStdString(popup.GetProperty(Demo::Popup::Property::DISPLAY_STATE));
