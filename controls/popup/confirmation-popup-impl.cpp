...
BaseHandle Create()
{
  return Demo::ConfirmationPopup::New();
}

DALI_TYPE_REGISTRATION_BEGIN(Demo::ConfirmationPopup, Demo::Popup, Create)

DALI_PROPERTY_REGISTRATION(Demo, ConfirmationPopup, ControlDetails[0].connectSignalPropertyName, STRING, CONNECT_SIGNAL_OK_SELECTED)
DALI_PROPERTY_REGISTRATION(Demo, ConfirmationPopup, ControlDetails[1].connectSignalPropertyName, STRING, CONNECT_SIGNAL_CANCEL_SELECTED)
...
SignalConnectorType signalConnector1(typeRegistration, Dali::String(ControlDetails[0].signalName), &Demo::Internal::ConfirmationPopup::DoConnectSignal);
SignalConnectorType signalConnector2(typeRegistration, Dali::String(ControlDetails[1].signalName), &Demo::Internal::ConfirmationPopup::DoConnectSignal);

DALI_TYPE_REGISTRATION_END()

} // Unnamed namespace

Dali::Demo::ConfirmationPopup ConfirmationPopup::New()
{
  // Create the implementation, temporarily owned on stack.
  IntrusivePtr<ConfirmationPopup> internalConfirmationPopup = new ConfirmationPopup();

  // Pass ownership to CustomActor
  Dali::Demo::ConfirmationPopup confirmationPopup(*internalConfirmationPopup);
...
: Demo::Internal::Popup()
{
  ...
  mControlSignalNames[Demo::ConfirmationPopup::CONTROL_OK]     = DEFAULT_CONNECT_SIGNAL_NAME;
  mControlSignalNames[Demo::ConfirmationPopup::CONTROL_CANCEL] = DEFAULT_CONNECT_SIGNAL_NAME;
}
...
{
  Demo::ConfirmationPopup popup = Demo::ConfirmationPopup::DownCast(Dali::BaseHandle(object));
  ...
  if(popup)
  {
    ...
    {
      case Demo::ConfirmationPopup::Property::CONNECT_SIGNAL_OK_SELECTED:
      {
        popupImpl.SetControlSignalName(Demo::ConfirmationPopup::CONTROL_OK, ToStdString(value));
        break;
      }
      case Demo::ConfirmationPopup::Property::CONNECT_SIGNAL_CANCEL_SELECTED:
      {
        popupImpl.SetControlSignalName(Demo::ConfirmationPopup::CONTROL_CANCEL, ToStdString(value));
        break;
      }
    }
  }
...
Property::Value ConfirmationPopup::GetProperty(BaseObject* object, Property::Index propertyIndex)
{
  ...
  if(popup)
  {
    ...
    switch(propertyIndex)
    {
      case Demo::ConfirmationPopup::Property::CONNECT_SIGNAL_OK_SELECTED:
      {
        value = ToPropertyValue(popupImpl.GetControlSignalName(Demo::ConfirmationPopup::CONTROL_OK));
        break;
      }
      case Demo::ConfirmationPopup::Property::CONNECT_SIGNAL_CANCEL_SELECTED:
      {
        value = ToPropertyValue(popupImpl.GetControlSignalName(Demo::ConfirmationPopup::CONTROL_CANCEL));
        break;
      }
    }
  }
...
bool ConfirmationPopup::DoConnectSignal(BaseObject* object, ConnectionTrackerInterface* tracker, const Dali::String& signalName, FunctorDelegate* functor)
{
  ...
  Demo::ConfirmationPopup popup = Demo::ConfirmationPopup::DownCast(handle);

  // Look up the requested signal, attempting to create it dynamically if it doesn't exist.
  SignalDelegate* signalDelegate = Dali::Demo::GetDerivedImplementation(popup).GetControlSignal(std::string(signalName.CStr()));
  if(signalDelegate)
  {
