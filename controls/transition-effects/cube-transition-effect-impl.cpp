...
// INTERNAL INCLUDES
#include <controls/shaders/cube-transition-effect-frag.h>
#include <controls/shaders/cube-transition-effect-vert.h>
...
DALI_TYPE_REGISTRATION_BEGIN(Demo::CubeTransitionEffect, Dali::BaseHandle, NULL);

DALI_SIGNAL_REGISTRATION(Demo, CubeTransitionEffect, "transitionCompleted", SIGNAL_TRANSITION_COMPLETED)
...
{
  Geometry geometry = Toolkit::VisualFactory::Get().CreateQuadGeometry();
...
void CubeTransitionEffect::OnTransitionFinished(Animation& source)
{
  ...
  // Emit signal
  Demo::CubeTransitionEffect handle(GetOwner());
  mTransitionCompletedSignal.Emit(handle, mCurrentTexture);
}

Demo::CubeTransitionEffect::TransitionCompletedSignalType& CubeTransitionEffect::TransitionCompletedSignal()
{
...
bool CubeTransitionEffect::DoConnectSignal(BaseObject* object, ConnectionTrackerInterface* tracker, const Dali::String& signalName, FunctorDelegate* functor)
{
  ...
  bool                       connected(true);
  Demo::CubeTransitionEffect cubeTransitionEffect = Demo::CubeTransitionEffect::DownCast(handle);
